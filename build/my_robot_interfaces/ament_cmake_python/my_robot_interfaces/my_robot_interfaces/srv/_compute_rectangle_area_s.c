...
bool my_robot_interfaces__srv__compute_rectangle_area__request__convert_from_py(PyObject * _pymsg, void * _ros_message)
{
  ...
  {  // widht
    PyObject * field = PyObject_GetAttrString(_pymsg, "widht");
    if (!field) {
      return false;
    }
    assert(PyFloat_Check(field));
    ros_message->widht = PyFloat_AS_DOUBLE(field);
    Py_DECREF(field);
  }
...
PyObject * my_robot_interfaces__srv__compute_rectangle_area__request__convert_to_py(void * raw_ros_message)
{
  ...
  {  // widht
    PyObject * field = NULL;
    field = PyFloat_FromDouble(ros_message->widht);
    {
      int rc = PyObject_SetAttrString(_pymessage, "widht", field);
      Py_DECREF(field);
      if (rc) {
        return NULL;
      }
